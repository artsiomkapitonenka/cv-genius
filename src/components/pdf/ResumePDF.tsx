...
const styles = StyleSheet.create({
  ...
  // Styles for additional pages with two columns
  additionalPage: {
...
// Components for individual parts of the resume
...
// Component for logo directly in SVG
const Logo = () => (
...
// Function to split projects into pages
const splitProjectsForPages = (projects: Project[]) => {
  const firstPageProjects = projects.slice(0, 1); // First project on the first page
  const additionalPagesProjects = projects.slice(1); // Remaining projects on subsequent pages
  
  // Split remaining projects into even and odd for left and right columns
...
export const ResumePDF = ({ data }: { data: ResumeData }) => {
  ...
  return (
    <Document>
      ...
      {/* Additional pages with two columns for remaining projects */}
