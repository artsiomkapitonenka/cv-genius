...
export const useResumeData = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!chatGPTResponse.ok) {
        // Получаем данные ответа даже если был ошибочный статус
        const errorData = await chatGPTResponse.json();
        const errorMessage = `API returned an error: ${chatGPTResponse.status}`;
        setApiErrorDetails(errorData?.error || JSON.stringify(errorData));
  ...
  return { data, loading, error, apiErrorDetails, updateData, retry };
};
